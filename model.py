...
class Color2Sketch(nn.Module):
    def __init__(self, nc=3, pretrained=False):
        ...
        if pretrained:
            ...
            checkpoint = torch.load('./checkpoint/color2edge/ckpt.pth',map_location=torch.device('cpu'))
...
class Sketch2Color(nn.Module):
    def __init__(self, nc=3, pretrained=False):
        ...
        if pretrained:
            ...
            assert os.path.isdir('checkpoint'), 'Error: no checkpoint directory found!'
            checkpoint = torch.load('./checkpoint/edge2color/ckpt.pth',map_location=torch.device('cpu'))
            self.load_state_dict(checkpoint['netG'],map_location=torch.device('cpu'),strict=True)
            print("Done!")
        else:
            self.apply(weights_init)
